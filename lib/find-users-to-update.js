...
module.exports = options => {
  ...
  options.masterdata.forEach(item => {
    masterObject[item.personalIdNumber] = item.department
  })
  ...
  const idsForUpdate = updates.map(item => item.personalIdNumber)
  const isReadyForUpdate = item => idsForUpdate.indexOf(item.personalIdNumber) > -1
